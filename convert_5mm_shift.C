...
void convert_5mm_shift(){
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      ...
      if (dis[i] == 0){
	filename.Form("./processed_data/5mm_shift/5mm_%1.0f_U%d.dat",dis[i],dis1[j]);
      }else{
	filename.Form("./processed_data/5mm_shift/5mm_%2.1f_U%d.dat",dis[i],dis1[j]);
      }
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      if (dis[i] == 0){
	filename.Form("./processed_data/5mm_shift/5mm_%1.0f_V%d.dat",dis[i],dis1[j]);
      }else{
	filename.Form("./processed_data/5mm_shift/5mm_%2.1f_V%d.dat",dis[i],dis1[j]);
      }
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      if (dis[i] == 0){
	filename.Form("./processed_data/5mm_shift/5mm_%1.0f_Y%d.dat",dis[i],dis1[j]);
      }else{
	filename.Form("./processed_data/5mm_shift/5mm_%2.1f_Y%d.dat",dis[i],dis1[j]);
      }
  ...
  TFile *file = new TFile("5mm_shift.root","RECREATE");
