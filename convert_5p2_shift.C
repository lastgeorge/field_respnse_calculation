...
void convert_5p2_shift(){
  ...
  TString filename;
   ...
   for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/5.2_degree_shift/Sim_5.2_U%d.dat",wire_num[i]);
   ...
   for (Int_t i=0;i!=nwire;i++){
     filename.Form("processed_data/5.2_degree_shift/Sim_5.2_V%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gv_i[i] = new TGraph();
    gv_c[i] = new TGraph();
  ...
  for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/5.2_degree_shift/Sim_5.2_Y%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gw_i[i] = new TGraph();
    gw_c[i] = new TGraph();
  ...
  TFile *file = new TFile("Sim_5p2_shift.root","RECREATE");
  for(Int_t i=0;i!=nwire;i++){
