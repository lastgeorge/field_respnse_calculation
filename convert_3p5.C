...
void convert_3p5(){
  ...
  TString filename;
   ...
   for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/3.5_degree_corr/Sim_3.5_U%d.dat",wire_num[i]);
   ...
   for (Int_t i=0;i!=nwire;i++){
     filename.Form("processed_data/3.5_degree_corr/Sim_3.5_V%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gv_i[i] = new TGraph();
    gv_c[i] = new TGraph();
  ...
  for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/3.5_degree_corr/Sim_3.5_Y%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gw_i[i] = new TGraph();
    gw_c[i] = new TGraph();
  ...
  TFile *file = new TFile("Sim_3p5.root","RECREATE");
  for(Int_t i=0;i!=nwire;i++){
