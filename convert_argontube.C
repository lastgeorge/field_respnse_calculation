const Int_t nwire = 7;
void convert_argontube(){

  //  Double_t dis[6]={0,0.3,0.6,0.9,1.2,1.5};
  TString dis[6]={"0","0.3","0.6","0.9","1.2","1.5"};
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      ...
      filename = "./processed_data/argontube/3mm_" + dis[i] + Form("_V%d.dat",dis1[j]);
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      ...
      filename = "./processed_data/argontube/3mm_" + dis[i] + Form("_Y%d.dat",dis1[j]);
  ...
  TFile *file = new TFile("argontube.root","RECREATE");
