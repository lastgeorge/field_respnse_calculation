const Int_t nwire = 3;
void convert_1p7(){
  TString prefix = "Sim_1.7";
  Int_t wire_num[nwire] = {1,2,3};
  ...
  TString filename;
   ...
   for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/1.7_degree_corr/Sim_1.7_U%d.dat",wire_num[i]);
   ...
   for (Int_t i=0;i!=nwire;i++){
     filename.Form("processed_data/1.7_degree_corr/Sim_1.7_V%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gv_i[i] = new TGraph();
    gv_c[i] = new TGraph();
  ...
  for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/1.7_degree_corr/Sim_1.7_Y%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gw_i[i] = new TGraph();
    gw_c[i] = new TGraph();
  ...
  TFile *file = new TFile("Sim_1p7.root","RECREATE");
  for(Int_t i=0;i!=nwire;i++){
