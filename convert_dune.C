const Int_t nwire = 7;
void convert_dune(){
  Double_t dis[6]={0,0.479,0.958,1.437,2.395};
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      ...
      if (dis[i] == 0){
	filename.Form("./processed_data/dune_fieldresp/dune_%1.0f_U%d.dat",dis[i],dis1[j]);
      }else{
	filename.Form("./processed_data/dune_fieldresp/dune_%4.3f_U%d.dat",dis[i],dis1[j]);
      }
      cout << filename << endl;
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      if (dis[i] == 0){
	filename.Form("./processed_data/dune_fieldresp/dune_%1.0f_V%d.dat",dis[i],dis1[j]);
      }else{
	filename.Form("./processed_data/dune_fieldresp/dune_%4.3f_V%d.dat",dis[i],dis1[j]);
      }
      cout << filename << endl;
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      if (dis[i] == 0){
	filename.Form("./processed_data/dune_fieldresp/dune_%1.0f_X%d.dat",dis[i],dis1[j]);
      }else{
	filename.Form("./processed_data/dune_fieldresp/dune_%4.3f_X%d.dat",dis[i],dis1[j]);
      }
      cout << filename << endl;
  ...
  TFile *file = new TFile("dune.root","RECREATE");
