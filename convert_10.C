...
void convert_10(){
  ...
  TString filename;
   ...
   for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/10_degree_corr/Sim_10_U%d.dat",wire_num[i]);
   ...
   for (Int_t i=0;i!=nwire;i++){
     filename.Form("processed_data/10_degree_corr/Sim_10_V%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gv_i[i] = new TGraph();
    gv_c[i] = new TGraph();
  ...
  for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/10_degree_corr/Sim_10_Y%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gw_i[i] = new TGraph();
    gw_c[i] = new TGraph();
  ...
  TFile *file = new TFile("Sim_10.root","RECREATE");
  for(Int_t i=0;i!=nwire;i++){
