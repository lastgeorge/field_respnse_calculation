const Int_t nwire = 21;
void convert_uboone_10(){
  ...
  TGraph **gu_i = new TGraph*[126];
  TGraph **gv_i = new TGraph*[126];
  TGraph **gw_i = new TGraph*[126];
  
  TGraph **gu_c = new TGraph*[126];
  TGraph **gv_c = new TGraph*[126];
  TGraph **gw_c = new TGraph*[126];

  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      
      // if (dis[i] == 0){
      // 	filename.Form("./processed_data/ub_10/%1.0f_U%d.dat",dis[i],dis1[j]);
      // }else{
	filename.Form("./processed_data/ub_10/%2.1f_U%d.dat",dis[i],dis1[j]);
	//}
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      // if (dis[i] == 0){
      // 	filename.Form("./processed_data/ub_10/%1.0f_V%d.dat",dis[i],dis1[j]);
      // }else{
	filename.Form("./processed_data/ub_10/%2.1f_V%d.dat",dis[i],dis1[j]);
	//}
  ...
  for (Int_t i=0;i!=6;i++){
    for (Int_t j=0;j!=nwire;j++){
      // if (dis[i] == 0){
      // 	filename.Form("./processed_data/ub_10/%1.0f_Y%d.dat",dis[i],dis1[j]);
      // }else{
	filename.Form("./processed_data/ub_10/%2.1f_Y%d.dat",dis[i],dis1[j]);
	//}
  ...
  TFile *file = new TFile("uboone.root","RECREATE");
