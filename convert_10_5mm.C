const Int_t nwire = 7;
void convert_10_5mm(){
  TString prefix = "Sim_10";
  Int_t wire_num[nwire] = {1,2,3,4,5,6,7};
  ...
  TString filename;
   ...
   for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/10_degree_5mm/Sim_10_5mm_U%d.dat",wire_num[i]);
   ...
   for (Int_t i=0;i!=nwire;i++){
     filename.Form("processed_data/10_degree_5mm/Sim_10_5mm_V%d.dat",wire_num[i]);
     ifstream infile(filename);
    Double_t x,y,num;
    gv_i[i] = new TGraph();
    gv_c[i] = new TGraph();
  ...
  for (Int_t i=0;i!=nwire;i++){
    filename.Form("processed_data/10_degree_5mm/Sim_10_5mm_Y%d.dat",wire_num[i]);
    ifstream infile(filename);
    Double_t x,y,num;
    gw_i[i] = new TGraph();
    gw_c[i] = new TGraph();
  ...
  TFile *file = new TFile("Sim_10_5mm.root","RECREATE");
  for(Int_t i=0;i!=nwire;i++){
